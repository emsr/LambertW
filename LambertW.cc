...
namespace utl {
  ...
  template<typename Float, int Branch, int Order>
    struct LogRecursionImpl
    {
      ...
      static Float
      Step(const Float logsx)
      {
        using LRI = LogRecursionImpl<Float, Branch, Order - 1>;
        return logsx - std::log(eSign * LRI::Step(logsx));
      }
    };
  ...
  template<typename Float, int Branch>
    ...
    public:
      ...
      template<int Order>
	static Float
	BranchPointExpansion(const Float x)
	{
          using Hrnr = Horner<Float, BranchPoint, Order>;
          return Hrnr::Eval(eSign * std::sqrt(2 * (Float(m_e) * x + 1)));
        }
	...
	{
	  const Float logsx = std::log(eSign * x);
	  const Float logslogsx = std::log(eSign * logsx);
  ...
  template<typename Float,
	...
	{
          static Float
          Recurse(const Float x, const Float w)
	...
	{
          static Float
          Recurse(const Float, const Float w)
          { return w; }
	};
  ...
  template<typename Float>
    ...
    {
      static inline Float
      Approximation(const Float x)
      {
	return x * horner_big_end(x, 0.07066247420543414, 2.4326814530577687,
                                     6.39672835731526, 4.663365025836821,
                                     0.99999908757381)
                 / horner_big_end(x, 1.2906660139511692, 7.164571775410987,
                                    10.559985088953114, 5.66336307375819, 1.0);
      }
    };
  ...
  template<typename Float>
    struct Pade<Float, 0, 2>
    {
      ...
      Approximation(const Float x)
      {
        const Float y = std::log(Float(0.5) * x) - 2;
	return 2 + y * horner_big_end(y, 0.00006979269679670452,
                                         0.017110368846615806,
                                         0.19338607770900237,
                                         0.6666648896499793)
                     / horner_big_end(y, 0.0188060684652668,
                                         0.23451269827133317, 1.0);
      }
    };
  ...
  template<typename Float>
    struct Pade<Float, -1, 4>
    {
      ...
      Approximation(const Float x)
      {
        return horner_big_end(x, -2793.4565508841197, -1987.3632221106518,
                                   385.7992853617571, 277.2362778379572,
                                    -7.840776922133643)
             / horner_big_end(x, 280.6156995997829, 941.9414019982657,
                                 190.64429338894644, -63.93540494358966, 1.0);
      }
    };
  ...
  template<typename Float>
    struct Pade<Float, -1, 5>
    {
      ...
      Approximation(const Float x)
      {
        const Float y = log(-x);
	return -std::exp(
            horner_big_end(y, 0.16415668298255184, -3.334873920301941,
                              2.4831415860003747, 4.173424474574879)
          / horner_big_end(y, 0.031239411487374164, -1.2961659693400076,
                              4.517178492772906, 1.0)
	);
      }
    };
  ...
  template<typename Float>
    struct Pade<Float, -1, 6>
    {
      ...
      Approximation(const Float x)
      {
        const Float y = log(-x);
	return -std::exp(
            horner_big_end(y, 0.000026987243254533254, -0.007692106448267341,
                              0.28793461719300206, -1.5267058884647018,
                             -0.5370669268991288)
          / horner_big_end(y, 3.6006502104930343e-6, -0.0015552463555591487,
                              0.08801194682489769, -0.8973922357575583, 1.0)
	);
      }
    };
  ...
  struct Pade<Float, -1, 7>
  {
    ...
    {
      return -1 - sqrt(
        horner_big_end(x, 988.0070769375508, 1619.8111957356814,
                          989.2017745708083, 266.9332506485452,
                           26.875022558546036)
      / horner_big_end(x, -205.50469464210596, -270.0440832897079,
                          -109.554245632316, -11.275355431307334, 1.0)
      );
    }
  };
  ...
  template<>
    ...
    {
      typedef double d;
      using Iter = Iterator<double, HalleyStep<d>>;
      return Y5(
